...
public class CreditAccount implements CreditAccountService {
    ...
    public CreditAccount create(int creditAccountId, User user0, Bank bank0, String dateBegin, String dateEnd, int creditPeriod, int creditAmount, int monthPayment, Employee employee0, PaymentAccount paymentAccount) {
        this.creditAccountId = creditAccountId;
        this.user = user0;
        this.bank = bank0;
        this.dateBegin = dateBegin;
    ...
    public boolean depositMoney(int amount) {
        if (this.paymentAccount.getAvailableAmount() >= amount) {
            if (this.curCreditAmount < amount) {
                this.paymentAccount.updateAmount(amount - this.curCreditAmount);
            } else {
